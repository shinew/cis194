...
exactMatches secret guess = length $ filter (\(a,b) -> a == b) $ zip secret guess
...
matches secret guess = sum $ map comp $ zip (countColors secret) (countColors guess)
    where comp (s,g) = min s g
